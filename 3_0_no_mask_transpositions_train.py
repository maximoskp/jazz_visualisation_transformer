...
year = augmented_train['composition_date'].to_numpy()
year_min = np.min(year)
year_max = np.max(year)
year_norm = list( (year - year_min)/(year_max - year_min) )
# style categories
harmonic_style_dict, harmonic_style_idx = get_dictionary_and_idxs_from_pd_column( augmented_train['harmonic_style'] )
# style categories
form_dict, form_idx = get_dictionary_and_idxs_from_pd_column( augmented_train['form'] )
# tonality categories
tonality_dict, tonality_idx = get_dictionary_and_idxs_from_pd_column( augmented_train['tonality'] )
# composer categories
composer_dict, composer_idx = get_dictionary_and_idxs_from_pd_column( augmented_train['composer'] )
# genre categories
...
tf.keras.utils.plot_model(encoder_model, show_shapes=True, expand_nested=True, to_file='figs/transformer_no_mask_base.png')
...
def print_summary(s):
    with open('data/model_no_mask_summary.txt','a') as f:
        print(s, file=f)
...
tf.keras.utils.plot_model(model, show_shapes=True, expand_nested=True, to_file='figs/model_no_mask_expanded.png')
tf.keras.utils.plot_model(model, show_shapes=True, expand_nested=False, to_file='figs/model_no_mask_compressed.png')
