...
history_path = 'data/model_no_mask_transposition_train_history_log.csv'
initial_epoch = 0
history = None
...
checkpoint_path = 'models/no_maks_transpositions_1/model'
...
figs_path = 'figs/no_mask_transpositions/'
...
def preprocess(inputs, year, style, form, tonality, composer, genre):
    ...
    return features, (year, style, form, tonality, composer, genre)
...
model = keras.Model(inputs, outputs=[year_output, style_output, form_output, tonality_output, composer_output, genre_output])
...
model.compile(
    loss=["mean_squared_error", "sparse_categorical_crossentropy", "sparse_categorical_crossentropy", "sparse_categorical_crossentropy", "sparse_categorical_crossentropy", "sparse_categorical_crossentropy"],
...
with open(figs_path + 'stats_no_mask.txt', 'w') as f:
...
with open(figs_path + 'stats_no_mask.txt', 'a') as f:
...
for l in ['year_predictor_loss','form_predictor_loss','style_predictor_loss', 'tonality_predictor_loss', 'composer_predictor_loss', 'genre_predictor_loss']:
    history_normalized[l] = history_normalized[l]/np.max(history_normalized[l])
ax = history_normalized.plot(x='epoch', y=['year_predictor_loss', 'form_predictor_loss', 'style_predictor_loss', 'tonality_predictor_loss', 'composer_predictor_loss', 'genre_predictor_loss'])
ax.figure.savefig(figs_path + 'history_losses.png', dpi=300)

ax = history.plot(x='epoch', y=['form_predictor_sparse_categorical_accuracy', 'style_predictor_sparse_categorical_accuracy', 'tonality_predictor_sparse_categorical_accuracy', 'composer_predictor_sparse_categorical_accuracy', 'genre_predictor_sparse_categorical_accuracy'])
ax.figure.savefig(figs_path + 'history_accuracies.png', dpi=300)

print('exporting data')

no_mask_visualization_data = {
    'year': {
...
visualization_data_path =  'data/no_mask_visualization_data.pickle'
with open(visualization_data_path, 'wb') as handle:
    pickle.dump(no_mask_visualization_data, handle, protocol=pickle.HIGHEST_PROTOCOL)